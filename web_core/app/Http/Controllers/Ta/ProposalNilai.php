...
class ProposalNilai extends Controller {
  
	// -------------------------------------------------------------------------------------------
	// function separator
	// -------------------------------------------------------------------------------------------
	public function detail(Request $req){
    $modul = 'TaProposalNilai';
  ...
  public function update(Request $req){    
		...
		try {
			foreach ($penguji as $key => $value) {
				TaProposalPengujiMdl::where('id', $value['id'])->update(['nilai_angka'=>$value['val']]);
			}
			foreach ($pembimbing as $key => $value) {
				TaProposalPembimbingMdl::where('id', $value['id'])->update(['nilai_angka'=>$value['val']]);
			}
			$prep = TaProposalMdl::find($id);
			if (!$prep) {
				DB::rollBack();
