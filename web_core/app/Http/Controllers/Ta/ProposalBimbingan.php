...
class ProposalBimbingan extends Controller {
  
	// -------------------------------------------------------------------------------------------
	// function separator
	// -------------------------------------------------------------------------------------------
  public function list(Request $req){
    $modul = 'TaProposalBimbingan';
    ...
    $user_ses = $req->session()->get('user_ses');
		...
		$dosen_pembimbing = TaProposalPembimbingMdl::cmbJoinAll($id)->get();
		$tbl = TaProposalBimbinganMdl::listBimbingan($id, $f1)->get();

		return view('ta.proposal_bimbingan_list', [
			'get'=>$get,
      'tbl'=>$tbl,
      'var'=>$arr_page,
      'cmb'=>[
      ...
      'user_ses'=>$user_ses,
			...
			'ctr_path'=>$user_ses['active_app']['link'].'/proposal/bimbingan/'.$enc_id,
			'mylib'=>Mylib::class,
      'page_title'=>[
        'icon'=>'fas fa-list',
  ...
  public function form(Request $req){
    ...
    $user_ses = $req->session()->get('user_ses');
		...
		if ($act == 'edit' && $sid != '') {
			$get_bimbingan = TaProposalBimbinganMdl::where('id',$sid)->where('status_bimbingan','aktif')->first();
			if (!$get_bimbingan) {
				return redirect($user_ses['active_app']['link'].'/proposal');
			}
			$form = [
				'in0'=>$get_bimbingan->id,
				'in1'=>$get_bimbingan->ta_proposal_pembimbing_id,
		...
		return view('ta.proposal_bimbingan_form', [
			'get'=>$get,
			'form'=>$form,
			'cmb'=>[
				'dosen_pembimbing'=>$dosen_pembimbing,
			...
			'page_title'=>[
				...
				'links'=>[
				['title'=>'Proposal Tugas Akhir','active'=>''],
				['title'=>'Form Bimbingan Proposal','active'=>'active'],
				],
			],
		]);
  }
    ...
    $user_ses = $req->session()->get('user_ses');
		...
		$uploaded_file = '';
		$path_file = 'uploads/proposal/'.date('Y').'/'.date('m');
		if ($req->hasFile('in5')) {
			$file = $req->file('in5');
			$maxsize = 5 * 1024 * 1024;
		...
		if ($act == 'add') {
			$bimbingan_terakhir = TaProposalBimbinganMdl::lastBimbingan($proposal_id, $pembimbing_id)->first();
			...
			$prep = new TaProposalBimbinganMdl();
			$prep->created_at = date('Y-m-d H:i:s');
			$prep->created_by = $user_ses['id'];
			$prep->ta_proposal_id = $proposal_id;
			$prep->ta_proposal_pembimbing_id = $pembimbing_id;
			$prep->bimbingan_ke = $bimbingan_ke;
			$prep->status_bimbingan = 'aktif';
		} else {
			$prep = TaProposalBimbinganMdl::find($req->post('in0'));
			if (!$prep) {
  ...
  public function detail(Request $req){
    ...
    $user_ses = $req->session()->get('user_ses');
		...
		$get_bimbingan = TaProposalBimbinganMdl::getBimbinganByid($sid)->first();
		if (!$get_bimbingan) {
			return redirect($user_ses['active_app']['link'].'/proposal');
		}
  ...
  public function simpan(Request $req){    
    ...
    if (!$ta_jenis) {
      ...
      $prep->ta_jenis_kode = $ta_jenis->kode;
			...
			return response()->json(['status'=>'success', 'statusText'=>Mylib::pesan('success','save'), 'directto'=>url($user_ses['active_app']['link'].'/proposal/form/detail/'.$prep->id)]);
    ...
    $modul = 'TaProposalBimbinganByDosen';
    ...
    $user_ses = $req->session()->get('user_ses');
		...
		$mahasiswa_bimbingan = TaProposalPembimbingMdl::cmbMahasiswaBimbingan($user_ses['pegawai_id'], $f1)->get();
		$tbl = TaProposalBimbinganMdl::listBimbinganByDosen($user_ses['pegawai_id'], $f1, $f2)->get();

		return view('ta.proposal_bimbingan_list_bydosen', [
			'tbl'=>$tbl,
			'var'=>$arr_page,
			'cmb'=>[
			...
			'user_ses'=>$user_ses,
					'app_path'=>$user_ses['active_app']['link'],
					'ctr_path'=>$user_ses['active_app']['link'].'/proposal/bimbingan/dosen',
					'mylib'=>Mylib::class,
					'crypt'=>Crypt::class,
			'page_title'=>[
				'icon'=>'fas fa-list',
    ...
    $user_ses = $req->session()->get('user_ses');
		...
		$bimbingan = TaProposalBimbinganMdl::getBimbinganByid($id)->first();
		if (!$bimbingan) {
			return redirect($ctr_link);
		}
		$get = TaProposalMdl::getByid($bimbingan->ta_proposal_id)->first();
		if (!$get) {
			return redirect($ctr_link);
		}
		$bimbingan_ke = TaProposalBimbinganMdl::listBimbinganKe($bimbingan->ta_proposal_id, $user_ses['pegawai_id'])->get();
	...
	public function save_bimbingan_dosen(Request $req){
		...
		$prep = TaProposalBimbinganMdl::find($req->post('id'));
		if (!$prep) {
			return response()->json(['status'=>'info', 'statusText'=>Mylib::pesan('fail', 'custom', 'Data Bimbingan tidak ditemukan.')]);
		}
