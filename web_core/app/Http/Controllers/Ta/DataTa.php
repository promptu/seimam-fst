...
class DataTa extends Controller {
	...
	public function list(Request $req){
		...
		if ($user_ses['active_role']['id'] == '3') {
			$tbl = TaDataMdl::listMahasiswa($user_ses['mahasiswa_nim'])->paginate($ppg);
		} else {
			$tbl = TaDataMdl::list($f1,$f2, $f3, $f4, $f5)->paginate($ppg);
		}

		return view('ta.data.list', [
			'tbl'=>$tbl,
			'var'=>$arr_page,
			'cmb'=>[
				'ppg'=>Mylib::ppg(),
			...
			'user_ses'=>$user_ses,
					'ctr_path'=>$user_ses['active_app']['link'].'/data-ta',
					'mylib'=>Mylib::class,
					'crypt'=>Crypt::class,
			'page_title'=>[
				'icon'=>'fas fa-list',
	...
	public function form(Request $req){
		...
		if ($state == 'add') {
			...
			if ($id) {
				...
				$pembimbing = TaDataPembimbingMdl::joinAll($id)->get();
			} else {
				// return redirect($user_ses['active_app']['link'].'/data-ta');
			}
		} else {
	  ...
	  'user_ses'=>$user_ses,
			...
			'ctr_path'=>$user_ses['active_app']['link'].'/data-ta',
			'back_path'=>$user_ses['active_app']['link'].'/data-ta'.(($ctr_ses['page']) ? '?page='.$ctr_ses['page'] : ''), 
	  'cmb'=>[
  ...
  public function simpan(Request $req){    
	...
	if (!$ta_jenis) {
	  ...
	  $prep->ta_jenis_kode = $ta_jenis->kode;
			...
			return response()->json(['status'=>'success', 'statusText'=>Mylib::pesan('success','save'), 'directto'=>url($user_ses['active_app']['link'].'/data-ta/form/detail/'.$enc_id)]);
