...
class DataTaJadwalUjian extends Controller {
  
	// -------------------------------------------------------------------------------------------
	// function separator
	// -------------------------------------------------------------------------------------------
	public function list(Request $req){
    $modul = 'DataTaJadwalUjian';
    ...
    $tbl = TaDataMdl::listStatusBerkasValid($f1,$f2,$f3,$f4,$f5)->paginate($ppg);

		return view('ta.data.jadwal_ujian_list', [
      'tbl'=>$tbl,
      'var'=>$arr_page,
      'cmb'=>[
        'ppg'=>Mylib::ppg(),
      ...
      'user_ses'=>$user_ses,
			'ctr_path'=>$user_ses['active_app']['link'].'/jadwal-ujian-ta',
			'mylib'=>Mylib::class,
			'crypt'=>Crypt::class,
      'page_title'=>[
        'icon'=>'fas fa-list',
    ...
    $user_ses = $req->session()->get('user_ses');
		$ctr_ses = $req->session()->get($modul);
		$ctr_link = $user_ses['active_app']['link'].'/jadwal-ujian-ta';
		...
		return view('ta.data.jadwal_ujian_detail', [
			'get'=>$get,
      'user_ses'=>$user_ses,
      ...
      'page_title'=>[
        'icon'=>'fas fa-list',
        'bread'=>'Jadwal Ujian Tugas Akhir',
        'links'=>[
          ['title'=>'Tugas Akhir','active'=>''],
          ['title'=>'Detail Jadwal Ujian','active'=>'active'],
        ],
      ],
		]);
	}
  ...
  public function add_penguji(Request $req){    
	...
	public function delete_penguji(Request $req){
		$id = $req->post('id');
		$delete = TaDataPengujiMdl::where('id',$id)->delete();
		if ($delete) {
    ...
    $user_ses = $req->session()->get('user_ses');
		...
		return view('ta.data.jadwal_ujian_detail_mhs', [
			'get'=>$get,
      'user_ses'=>$user_ses,
			'app_path'=>$user_ses['active_app']['link'],
			'ctr_path'=>$ctr_link,
      ...
      'page_title'=>[
        ...
        'links'=>[
          ['title'=>'Tugas Akhir','active'=>''],
          ['title'=>'Jadwal Ujian','active'=>'active'],
        ],
      ],
		]);
	}
