    ...
    if ($user_ses['active_role']['id'] == '3') {
      ...
      if ($get_mahasiswa) {
		...
		if ($state == 'add') {
			...
			$get = [];
			$pembimbing = [];
			$page_view = 'ta.proposal_form_add';
		} else {
			...
			if ($state == 'edit') {
				if ($get->ta_status_pengajuan_kode != 'pengajuan') {
					return redirect($user_ses['active_app']['link'].'/proposal');
				}
				$page_view = 'ta.proposal_form_add';
				$state_page = 'update';
			} else {
				$page_view = 'ta.proposal_form_detail';
				$state_page = 'detail';
			}			
		}
  ...
  public function simpan(Request $req){    
    ...
    //     $datas = $body['data'];
      ...
      $prep->ta_jenis_kode = $ta_jenis->kode;
			...
			DB::table('ta_proposal_pembimbing')->insert([
				['ta_proposal_id'=>$id, 'pembimbing_ke'=>1, 'pegawai_id'=>$p1, 'is_active'=>'T', 'created_at'=>$dtm, 'created_by'=>$user_ses['id']],
				['ta_proposal_id'=>$id, 'pembimbing_ke'=>2, 'pegawai_id'=>$p2, 'is_active'=>'T', 'created_at'=>$dtm, 'created_by'=>$user_ses['id']],
				['ta_proposal_id'=>$id, 'pembimbing_ke'=>3, 'pegawai_id'=>$p3, 'is_active'=>'T', 'created_at'=>$dtm, 'created_by'=>$user_ses['id']],
				['ta_proposal_id'=>$id, 'pembimbing_ke'=>4, 'pegawai_id'=>$p4, 'is_active'=>'T', 'created_at'=>$dtm, 'created_by'=>$user_ses['id']],
			]);
			DB::commit();

			$enc_id = Crypt::encryptString($prep->id);
			return response()->json(['status'=>'success', 'statusText'=>Mylib::pesan('success','save'), 'directto'=>url($user_ses['active_app']['link'].'/proposal/form/edit/'.$enc_id)]);
	...
	public function add_pembimbing(Request $req){
		$validasi = Validator::make($req->all(), [
			...
			'dosen_ke'=>'required|numeric|min:1|max:4',
		], [
			'required'=>Mylib::validasi('required'),
			'numeric'=>Mylib::validasi('numeric'),
			'dosen_ke.min'=>'Minimal pembimbing ke 1',
			'dosen_ke.max'=>'Maksimal pembimbing ke 4',
		], [
		...
		// $cek_pembimbing = TaProposalPembimbingMdl::where('ta_proposal_id', $req->post('id'))->where('')
