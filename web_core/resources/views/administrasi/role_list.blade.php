		...
		<form action="{{ url($ctr_path) }}" method="post" id="fr0" autocomplete="off">
			...
			<div class="card card-outline card-warning">
				<div class="card-body pb-2 row">
					<div class="col-lg-6">
						<div class="row mb-2">
							<label for="f1" class="col-lg-4 label-fr">Nama Role :</label>
							<div class="col-lg-7">
					...
					<table class="table table-sm table-striped table-hover no-wrap">
						...
						<tbody>
							...
							<tr>
								...
								<td>{{ $r['nama'] }}</td>
								<td>{{ $r['urutan'] }}</td>
								<td>{!! $mylib::is_admin($r['is_admin'],'lbl') !!}</td>
        ...
        <div class="modal-body">
          ...
          <div class="row mb-2">
            <div class="col-lg offset-lg-4">
              <div class="icheck-secondary d-inline"><input type="checkbox" id="mdfr-in4"><label for="mdfr-in4">Akses Admin ?</label></div>
            </div>
          </div>
          <div class="row mb-2">
            <div class="col-lg offset-lg-4">
...
<script>
  ...
  $('#fr1').submit(function(e){
    ...
    const datas = {'_token':token,'act':act,'in0':$('#mdfr-in0').val(),'in1':$('#mdfr-in1').val(),'in2':$('#mdfr-in2').val(),'in3':($('#mdfr-in3').is(':checked')) ? 'Y' : 'T','in4':($('#mdfr-in4').is(':checked')) ? 'Y' : 'T'};
    const bid = $('#mdfr-btn');
    const bval = bid.html();
    $.ajax({
