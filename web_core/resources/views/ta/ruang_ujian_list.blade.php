		...
		<form action="{{ url($ctr_path) }}" method="post" id="fr0" autocomplete="off">
			...
			<div class="card card-outline card-warning">
				<div class="card-body pb-2 row">
					<div class="col-lg-6">
						<div class="row mb-2">
							<label for="f3" class="col-lg-4 label-fr">Unit :</label>
							<div class="col-lg-7">
                <select name="f3" id="f3" class="form-control select2 chform">
                @foreach ($cmb['unit'] as $c)
                <option value="{{ $c['id'] }}" {{ ($c['id'] == $var['f3']) ? 'selected' : '' }}>{!! $mylib::tree_view($c['val'], $c['level']) !!}</option>
                @endforeach
                </select>
							</div>
						</div>
					...
					<div class="col-lg-6">
						<div class="row mb-2">
							<label for="f1" class="col-lg-4 label-fr">Nama Ruang :</label>
							<div class="col-lg-7">
			...
			<div class="card card-outline card-secondary">
				<div class="card-body p-0 table-responsive">
                  ...
                  @else
									<a href="#" class="bedit btn btn-xs btn-info" data-id="{{ $r['kode'] }}"><i class="fas fa-edit"></i></a>
									<a href="#" class="bdel btn btn-xs btn-danger" data-id="{{ $r['kode'] }}" data-nm="{{ $r['nama'] }}"><i class="fas fa-trash-alt"></i></a>
                  @endif
								</td>
							</tr>
							@endforeach
	...
	<div class="modal-dialog">
      ...
      <form id="fr1" action="#" method="post" autocomplete="off">
        ...
        <div class="modal-body">
          <div class="row mb-2">
            <label for="mdfr-in4" class="col-lg-4 label-fr">Unit :</label>
            <div class="col-lg-7">
              <select name="mdfr-in4" id="mdfr-in4" class="form-control select2"><option value="">- Pilih -</option>
              @foreach ($cmb['unit'] as $c)
          ...
          <div class="row mb-2">
            <label for="mdfr-in1" class="col-lg-4 label-fr">Kode :</label>
            <div class="col-lg-4"><input type="text" name="mdfr-in1" id="mdfr-in1" class="form-control"></div>
          </div>
          <div class="row mb-2">
            <label for="mdfr-in2" class="col-lg-4 label-fr">Nama Ruang :</label>
...
<script>
  ...
  $('#fr1').submit(function(e){
    ...
    const datas = {'_token':token,'act':act,'in0':$('#mdfr-in0').val(), 'in1':$('#mdfr-in1').val(), 'in2':$('#mdfr-in2').val(), 'in3':($('#mdfr-in3').is(':checked')) ? 'Y' : 'T', 'in4':$('#mdfr-in4').val()};
    const bid = $('#mdfr-btn');
    const bval = bid.html();
    $.ajax({
    ...
    if (state == 'edit') {
      $('#mdfr-txt').text('EDIT')
      $('#mdfr-act').val('update');
      $('#mdfr-in1').val(datas[0]).attr('disabled', true);
      $('#mdfr-in2').val(datas[1]);
      $('#mdfr-in3').prop('checked', (datas[2] == 'Y') ? true : false); 
      $('#mdfr-in4').val(datas[3]).trigger('change');
    } else {
      $('#mdfr-txt').text('TAMBAH')
      $('#mdfr-act').val('save');
      $('#mdfr-in1').val('').attr('disabled', false);
      $('#mdfr-in2').val('');
      $('#mdfr-in3').prop('checked', true);
      $('#mdfr-in4').val("{{ $var['f3'] }}").trigger('change');
    }
