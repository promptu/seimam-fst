					...
					<div class="card-body p-0 table-responsive">
						<table class="table table-hover table-striped table-sm">
							...
							<tbody>
								...
								@php
								    $detail = \App\Models\TaProposalSyaratUjianUploadMdl::where('ta_proposal_id', $id_proposal)->where('ta_proposal_syarat_ujian_id', $r['id'])->first();
								@endphp
								<tr>
									<td>{{ ++$no }}</td>
									<td>{{ $r['nama'] }}</td>
									<td>										
										@if ($detail)
										<a href="#" class="bview btn btn-info" data-nm="{{ $r['nama'] }}" data-url="{{ url($detail->berkas) }}"><i class="fas fa-eye"></i> Tampilkan</a>
										@else	
										<span class="badge badge-info"><i class="fas fa-exclamation-triangle"></i> Belum diunggah</span>
										@endif
									</td>
									<td> 
										
										@if ($get['status_berkas'] == 'VALID' || $get['status_berkas'] == 'INVALID')
										{!! $mylib::is_valid((($detail) ? $detail->is_valid : ''), 'lbl') !!} 
										@else
										<select class="form-control optval" name="optval-{{ $r['id'] }}" id="optval-{{ $r['id'] }}"><option value="">- Pilih -</option>
											@foreach ($mylib::is_valid() as $c)
											<option value="{{ $c['id'] }}" {{ ($c['id'] == $detail->is_valid && $get['status_berkas'] != 'PENGAJUAN') ? 'selected' : '' }}>{{ $c['val'] }}</option>
											@endforeach
										</select>
										@endif
									</td>
								...
								@endforeach
								@if ($no == 0)
								<tr><td colspan="5" align="center">Syarat ujian belum ditetapkan untuk program studi <b>"{{ $get['prodi_nama'] }}"</b>, Jenis Ta. <b>"{{ $get['ta_jenis_nama'] }}"</b></td></tr>
								@endif
							</tbody>
						</table>
					</div>
					<div class="card-footer p-2 row">
						<div class="col-lg-8"><b>Status Pengajuan : </b><br>@if ($get['status_berkas'] == 'PENGAJUAN') Menunggu validasi @elseif($get['status_berkas'] == 'VALID') Berkas Valid @elseif($get['status_berkas'] == 'INVALID') Berkas yang diajukan dinyatakan tidak valid @else Silahkan upload berkas sesuai persyaratan, dan ajukan berkas untuk divalidasi. @endif</div><div class="col-lg text-right"><a href="#" id="btn-aju" class="btn btn-primary ml-auto {{ ($get['status_berkas'] != 'PENGAJUAN') ? 'disabled' : '' }}"><i class="fas fa-cogs"></i> Proses Validasi</a></div>
					</div>
				</div>
			</div>
		</div>
...
<div id="mdaju" class="modal fade">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body p-3"><p class="mb-2">Kirim hasil validasi, lanjutkan ?</p><i>Perhatian : Cek kesesuaian berkas yang diunggah dan status validasi.</i></div>
      <div class="modal-footer justify-content-between p-2">
        ...
        <button id="mdaju-btn" class="btn btn-primary"><i class="fas fa-paper-plane"></i> Kirim Validasi</button>
      </div>
		</div>
	</div>
</div>
...
<script>
	...
	$('#mdaju-btn').click(function(e){
		...
		const formdata = {
			"_token":token, "id":"{{ $id_proposal }}", "in":cmbdata
		};
		console.log(formdata);
		
		$.ajax({
			url:"{{ url($ctr_path.'/save') }}", type:'POST', dataType:'json', data:formdata,
