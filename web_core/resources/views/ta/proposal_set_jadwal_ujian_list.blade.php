...
<div class="content">
	<div class="container">
        ...
        <div class="card-body pb-2 row">
					...
					<div class="col-lg-6">
						<div class="row mb-2">
							<label for="f4" class="col-lg-4 label-fr">NIM / Nama Mahasiswa :</label>
							<div class="col-lg-7">
								<input type="text" name="f4" id="f4" class="form-control" value="{{ $var['f4'] }}">
							</div>
						</div>
					</div>
					<div class="col-lg-6">
						<div class="row mb-2">
							<label for="f5" class="col-lg-4 label-fr">Status Ujian :</label>
							<div class="col-lg-7">
								<select name="f5" id="f5" class="chform form-control"><option value="">- Semua Status -</option>
									@foreach($cmb['status_lulus'] as $c) <option value="{{ $c['id'] }}" {{ ($c['id'] == $var['f5']) ? 'selected' : '' }}>{{ $c['val'] }}</option> @endforeach
								</select>
							</div>
						</div>
					</div>
          ...
          @endif
			...
			<div class="card card-outline card-secondary">
				<div class="card-body p-0 table-responsive">
                  ...
                  @php
                    $str_pembimbing = '';
                    $pembimbing = \App\Models\TaProposalPembimbingMdl::getByid($r['id'])->get();
                  ...
                  @endphp
								...
								<td>{!! $r['ta_ruang_ujian_nama'] !!}</td>
                <td>
                  @php
                    $tgl_ujian = "-"; $jam_mulai = "-"; $jam_selesai = "-";
                    if ($r['tgl_ujian_mulai']) {
                      $exp = explode(" ", $r['tgl_ujian_mulai']);
                      if (count($exp) == 2) {
                        $tgl_ujian = $exp[0];
                    ...
                    if ($r['tgl_ujian_selesai']) {
                      $exp = explode(" ", $r['tgl_ujian_selesai']);
                      if (count($exp) == 2) {
                        $exp_jam = explode(":", $exp[1]);
                        if (count($exp_jam) == 3) {
                    ...
                    echo $tgl_ujian." &nbsp; ".$jam_mulai." s/d ".$jam_selesai;
                  @endphp
                </td>
								<td>
                  @if ($r['is_edit'] == 'T')
                  ...
                  @else
                  <a href="{{ url($ctr_path.'/detail/'.$crypt::encryptString($r['id'])) }}" class="btn btn-xs btn-primary" data-id="{{ $r['id'] }}"><i class="fas fa-eye"></i></a>
                  @endif
								</td>
							</tr>
							@endforeach
