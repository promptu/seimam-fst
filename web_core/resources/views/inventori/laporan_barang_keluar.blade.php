	...
	<div class="col-lg-12">
		<div class="card card-outline card-maroon">
			...
			<div class="card-body">
				...
				<div class="row mb-3">
					<label for="f2" class="col-sm-3">Status Pengajuan :</label>
					<div class="col-sm">
						<select name="f2" id="f2" class="form-control"><option value="">-  Semua Status -</option>
                ...
                <div class="row mb-3">
					<label for="f3" class="col-sm-3">Status Barang :</label>
					<div class="col-sm">
						<select name="f3" id="f3" class="form-control"><option value="">-  Semua Status -</option>
						@foreach ($cmb['status_pengajuan_detail'] as $c)
							@if ($c['id'] != 'draft' && $c['id'] != 'revisi')
							<option value="{{$c['id']}}">{{$c['val']}}</option>
							@endif
						@endforeach						
						</select>
				...
				<div class="row mb-3">
                    <label for="f4_start" class="col-sm-3">Tanggal Verifikasi (Mulai):</label>
                    <div class="col-sm">
                        <input type="date" name="f4_start" id="f4_start" class="form-control datepicker" placeholder="Tanggal Mulai" autocomplete="off">
                    </div>
                </div>
                <div class="row mb-3">
                    <label for="f4_end" class="col-sm-3">Tanggal Verifikasi (Selesai):</label>
                    <div class="col-sm">
				...
				<div class="row mb-3">
					...
					<div class="col-sm">
                        <a href="{{url('/inventori/laporan/barang-keluar/html')}}" class="bexport btn btn-primary mr-3"><i class="fab fa-html5"></i> HTML</a>						
						{{-- <a href="{{url('/inventori/laporan/barang-keluar/excel')}}" class="bexport btn btn-info mr-3"><i class="far fa-file-excel"></i> Excel</a></div> --}}
				</div>
			</div>
			</form>
		</div>
...
<script>
	...
	$('.bexport').click(function(e){
		...
		const bid = $(this);
		$.ajax({url:"{{url('/inventori/laporan/barang-keluar/set-filter')}}", type:'post', dataType:'json', data:{'_token':token,'f1':f1,'f2':f2,'f3':f3,'f4_start':f4_start,'f4_end':f4_end},
