				...
				<div class="card-body pb-2 row">
					<div class="col-lg-6">
						<div class="row mb-2">
							...
							<div class="col-lg-7">
								<select name="f1" id="f1" class="chform form-control select2">
					...
					<div class="col-lg-6">
						<div class="row mb-2">
							<label for="f2" class="col-lg-4 label-fr">Template/Jenis Surat :</label>
							<div class="col-lg-7">
								<select name="f2" id="f2" class="chform form-control"><option value="">- Semua -</option>
									@foreach($cmb['template'] as $c) <option value="{{ $c['id'] }}" {{ ($c['id'] == $var['f2']) ? 'selected' : '' }}>{{ $c['val'] }}</option> @endforeach
								</select>
							</div>
						</div>
					</div>
      ...
      <form id="fr1" action="#" method="post" autocomplete="off">
        ...
        <div class="modal-body">
          <div class="row mb-2">
            ...
            <div class="col-lg-7">
              <select name="mdfr-in1" id="mdfr-in1" class="form-control select2" style="width: 100%">
                @foreach($cmb['unit_kerja'] as $c) <option value="{{ $c['id'] }}">{{ $c['val'] }}</option> @endforeach
              </select>
            </div>
          </div>
          <div class="row mb-2">
            <label for="mdfr-in2" class="col-lg-4 label-fr">Template/Jenis Surat :</label>
            <div class="col-lg-7">
              <select name="mdfr-in2" id="mdfr-in2" class="form-control" style="width: 100%"><option value="">- Pilih -</option>
                @foreach($cmb['template'] as $c) <option value="{{ $c['id'] }}">{{ $c['val'] }}</option> @endforeach
              </select>
            </div>
          </div>
          <div class="row mb-2">
            <label for="mdfr-in3" class="col-lg-4 label-fr">Syarat Pengajuan :</label>
            <div class="col-lg-7">
              <input type="text" name="mdfr-in3" id="mdfr-in3" class="form-control" value="">
            </div>
          </div>
          <div class="row mb-2">
            <div class="col-lg offset-lg-4">
              <div class="icheck-secondary d-inline"><input type="checkbox" id="mdfr-in4"><label for="mdfr-in4">Aktif ?</label></div>
            </div>
          </div>
        </div>
...
<script>
  ...
  $('#fr1').submit(function(e){
    ...
    const datas = {'_token':token, 'act':act, 'in0':$('#mdfr-in0').val(), 'in1':$('#mdfr-in1').val(), 'in2':$('#mdfr-in2').val(), 'in3':$('#mdfr-in3').val(),'in4':($('#mdfr-in4').is(':checked')) ? 'Y' : 'T'};
    const bid = $('#mdfr-btn');
    const bval = bid.html();
    $.ajax({
