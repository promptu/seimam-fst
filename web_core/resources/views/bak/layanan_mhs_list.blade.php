		...
		<form action="{{ url($ctr_path) }}" method="post" id="fr0" autocomplete="off">
			...
			<div class="card card-outline card-warning">
				<div class="card-body pb-2 row">
					<div class="col-lg-6">
						<div class="row mb-2">
							<label for="f1" class="col-lg-4 label-fr">Jenis Surat :</label>
							<div class="col-lg-7">
								<select name="f1" id="f1" class="chform form-control"><option value="">- Semua -</option>
									@foreach($cmb['template'] as $c) <option value="{{ $c['id'] }}" {{ ($c['id'] == $var['f1']) ? 'selected' : '' }}>{{ $c['val'] }}</option> @endforeach
								</select>
							</div>
						</div>
					</div>
				...
				<div class="card-body p-0 table-responsive">
					<table class="table table-sm table-striped table-hover no-wrap">
						...
						<tbody>
							...
							<tr>
								<td>{{ ++$no }}</td>
								<td>{{ $r['bak_template_nama'] }}</td>
								<td>{{ $r['keperluan'] }}</td>
								<td>{{ $mylib::indotgl($r['tgl_pengajuan']) }}</td>
								<td>{!! $mylib::bak_status_pengajuan($r['status'], 'lbl') !!}</td>
								<td>{!! $mylib::status_berkas($r['is_syarat_valid'], 'lbl') !!}</td>
								<td>
                  @if ($r['is_edit'] == 'T')
									<a href="#" class="btn btn-xs btn-secondary disabled"><i class="fas fa-lock"></i></a>
                  @elseif(in_array($r['status'], ['DRAFT', 'PENGAJUAN']))
                  ...
                  @elseif($r['status'] == 'SELESAI')
									<a href="{{ url($ctr_path.'/print/'.$r['id']) }}" class="btn btn-xs btn-secondary"><i class="far fa-file-pdf"></i></a>
                  @endif
									<a href="{{ url($ctr_path.'/detail/'.$crypt::encryptString($r['id'])) }}" class="btn btn-xs btn-primary"><i class="fas fa-eye"></i> Detail</a>
								</td>
							</tr>
							@endforeach
							@if ($no == 0)
	...
	<div class="modal-dialog">
      ...
      <form id="fr1" action="#" method="post" autocomplete="off">
        ...
        <div class="modal-body">
          <div class="row mb-2">
            <label for="mdfr-in1" class="col-lg-4 label-fr">Jenis Surat :</label>
            <div class="col-lg-7">
              <select name="mdfr-in1" id="mdfr-in1" class="form-control" style="width: 100%"><option value="">- Pilih -</option>
                @foreach($cmb['template'] as $c) <option value="{{ $c['id'] }}">{{ $c['val'] }}</option> @endforeach
              </select>
            </div>
          </div>
          <div class="row mb-2">
            <label for="mdfr-in2" class="col-lg-4 label-fr">Keperluan :</label>
            <div class="col-lg-7">
              <input type="text" name="mdfr-in2" id="mdfr-in2" class="form-control" value="">
            </div>
          </div>
        </div>
...
<script>
  ...
  $('#fr1').submit(function(e){
    ...
    const datas = {'_token':token, 'act':act, 'in0':$('#mdfr-in0').val(), 'in1':$('#mdfr-in1').val(), 'in2':$('#mdfr-in2').val()};
    const bid = $('#mdfr-btn');
    const bval = bid.html();
    $.ajax({
